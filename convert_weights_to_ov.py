#!/usr/bin/env python3
"""
Script to covert a .h5 weights file of the DTLN model to saved model for OpenVINO.

Example call:
    $python convert_weights_to_ov.py -m /name/of/the/model.h5 \
                                              -t name_target 

Go to $target_folder_1 and $target_folder_2 to run model optimizer

    $python "c:\Program Files (x86)\intel\openvino_2021.4\deployment_tools\model_optimizer"\mo.py
       --saved_model_dir . --disable_nhwc_to_nchw --model_name DTLN_norm_500h
...
if __name__ == '__main__':
    ...
    print("norm_stft:{}".format(norm_stft))
    # build model    
    ...
    # convert first model
    temp_model_file = args.target_folder + '_1'
    tf.saved_model.save(model_1,temp_model_file)
    # convert second model
    temp_model_file = args.target_folder + '_2'
    tf.saved_model.save(model_2,temp_model_file)
    
          
    print('Saved Model for OpenVINO conversion complete!')